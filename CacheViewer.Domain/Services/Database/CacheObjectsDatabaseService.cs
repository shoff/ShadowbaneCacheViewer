        ...
        public async Task SaveToDatabaseAsync(int validRange)
        {
            ...
            using (var context = new SbCacheViewerContext())
            {
                ...
                foreach (var i in this.cacheObjectFactory.Indexes)
                {
                    ...
                    foreach (var renderId in cobject.RenderIds)
                    {
                        robjectsCount++;
                        var rentity = context.RenderEntities.FirstOrDefault(r => r.CacheIndexIdentity == renderId);
                        if (rentity != null)
                        {
                            centity.RenderEntities.Add(rentity);
                        }
                    }
                    ...
                    if (save == 20)
                    {
                        ...
                        {
                            CacheObjectsCount = cobjectsCount,
                            RenderOffsetsCount = robjectsCount
                        };
