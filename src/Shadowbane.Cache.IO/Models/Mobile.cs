namespace Shadowbane.Cache.IO.Models
{
    ...
    {
        public Mobile(CacheIndex cacheIndex, ObjectType flag, string name, uint offset, ReadOnlyMemory<byte> data,
            uint innerOffset)
        ...
        public override void Parse()
        {
            ...
            this.WolfpackCreateDate = DateTime.UnixEpoch.AddMilliseconds(time);//((double)time).FromUnixTimeStamp();
            ...
            if (finder?.NullTerminator > 0)
            {
                for (int i = 0; i < finder?.NullTerminator; i++)
                {
                    if (reader.CanRead(8))
                    {
                        ...
                        // WTF file am I reading here?
                        // why is this object looking this up?
                        if (CacheIndexLookup.Instance.IsValidId(id))
                        {
    ...
    */
        ...
        public MobileRenderFinder GetFinder(BinaryReader reader)
        {
            ...
            if(CacheIndexLookup.Instance.IsValidId(finder.RenderId))
            {
                // TODO THIS IS ALMOST CERTAINLY WRONG MUST HAVE UNIT TESTS
